<#
.SYNOPSIS
    This PowerShell script disables "Always install with elevated privileges" for the Windows Installer.
...
try {
    ...
    if ($newValue -eq $DesiredValue) {
        Write-Host "SUCCESS: '$ValueName' set to $DesiredValue (Disabled) at $RegPath" -ForegroundColor Green
    } else {
        Write-Host "WARNING: Value did not apply as expected. Current value: $newValue" -ForegroundColor Red
    }
}
catch {
    Write-Host "ERROR: Failed to remediate WN11-CC-000315." -ForegroundColor Red
