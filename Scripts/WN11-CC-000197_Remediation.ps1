<#
.SYNOPSIS
    This PowerShell script enables "Turn off Microsoft consumer experiences" by disabling Windows consumer features.
...
.USAGE
    Example syntax:
    PS C:\> .\WN11-CC-000197_Remediation.ps1
#>
...
try {
    ...
    # Set the value to 1 (Enabled - Turn off Microsoft consumer experiences)
    New-ItemProperty -Path $RegPath -Name $ValueName -Value $DesiredValue -PropertyType DWord -Force | Out-Null

    # Verify
    $newValue = (Get-ItemProperty -Path $RegPath -Name $ValueName).$ValueName
    if ($newValue -eq $DesiredValue) {
        Write-Host "SUCCESS: '$ValueName' set to $DesiredValue (Turn off Microsoft consumer experiences: Enabled) at $RegPath" -ForegroundColor Green
    } else {
        Write-Host "WARNING: Value did not apply as expected. Current value: $newValue" -ForegroundColor Red
    }
}
catch {
    Write-Host "ERROR: Failed to remediate WN11-CC-000197." -ForegroundColor Red
