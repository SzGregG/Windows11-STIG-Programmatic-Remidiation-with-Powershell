<#
.SYNOPSIS
    This PowerShell script enables "Turn off printing over HTTP" by disabling HTTP printing.
...
try {
    ...
    # Set the value to 1 (Enabled - Turn off printing over HTTP)
    New-ItemProperty -Path $RegPath -Name $ValueName -Value $DesiredValue -PropertyType DWord -Force | Out-Null

    # Verify
    $newValue = (Get-ItemProperty -Path $RegPath -Name $ValueName).$ValueName
    if ($newValue -eq $DesiredValue) {
        Write-Host "SUCCESS: '$ValueName' set to $DesiredValue (Turn off printing over HTTP: Enabled) at $RegPath" -ForegroundColor Green
    } else {
        Write-Host "WARNING: Value did not apply as expected. Current value: $newValue" -ForegroundColor Red
    }
}
catch {
    Write-Host "ERROR: Failed to remediate WN11-CC-000110." -ForegroundColor Red
