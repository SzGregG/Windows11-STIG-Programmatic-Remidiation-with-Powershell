<#
.SYNOPSIS
    This PowerShell script enables "Require a password when a computer wakes (plugged in)".
...
.USAGE
    Example syntax:
    PS C:\> .\WN11-CC-000150_Remediation.ps1
#>

#Requires -RunAsAdministrator

# GUID 0e796bdb-100d-47d6-a2d5-f7d2daa51f51 = "Require a password when a computer wakes"
# ACSettingIndex governs the "Plugged in" (AC power) state
...
try {
    ...
    # Set the value to 1 (Enabled - Require password on wake, plugged in)
    New-ItemProperty -Path $RegPath -Name $ValueName -Value $DesiredValue -PropertyType DWord -Force | Out-Null

    # Verify
    $newValue = (Get-ItemProperty -Path $RegPath -Name $ValueName).$ValueName
    if ($newValue -eq $DesiredValue) {
        Write-Host "SUCCESS: '$ValueName' set to $DesiredValue (Require password when computer wakes, plugged in: Enabled) at $RegPath" -ForegroundColor Green
    } else {
        Write-Host "WARNING: Value did not apply as expected. Current value: $newValue" -ForegroundColor Red
    }
}
catch {
    Write-Host "ERROR: Failed to remediate WN11-CC-000150." -ForegroundColor Red
